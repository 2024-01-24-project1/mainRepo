...
public class LostArticleSearch {
	...
	private static void lostArticlePage(ArrayList<LostArticle> list) {
		// 리스트의 페이지수 계산
		...
		while(true) {
			...
			list.stream().skip(index * 10)
			 							 .limit(10)
			 							 .forEach(article -> System.out.printf("%-10s|%-20s|%-15s|%-15s\r\n"
					 													, article.getArticle()
					 													, article.getContent()
					 													, article.getLostStation()
					 													, article.getFindStation()));
